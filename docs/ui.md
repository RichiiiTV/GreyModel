...
These flags are intentionally explicit. They are meant for notebook or HPC proxy environments where the proxy path or origin handling breaks Streamlit's websocket negotiation. In `jupyter_port` mode the launcher uses the generic `/proxy/<port>/` route and does not auto-inject `server.baseUrlPath` unless you explicitly provide one.
