...
Mixed-resolution Hugging Face imports are shape-bucketed into separate pseudo-stations by default so small images are not padded to the largest image in the whole public dataset. Disable that only with `--no-shape-bucketed-stations`.
